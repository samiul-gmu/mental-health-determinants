...
@author: Sami (Updated for 10-Fold Cross-Validation with Support Vector Regression)
"""
...
# Function to perform 10-fold cross-validation using Support Vector Regression
def cross_validate_svr(X, y, target_name, kernel='rbf', C=1.0, epsilon=0.1, n_splits=10):
    ...
    print(f"\n10-Fold Cross-Validation Results for SVR ({target_name}, Kernel={kernel}, C={C}, Epsilon={epsilon}):")
    print(f"Average MSE: {avg_mse:.4f}")
    print(f"Average R²: {avg_r2:.4f}")

    # Save results to CSV
    save_results_to_csv('Support Vector Regression (10-Fold CV)', target_name, avg_mse, avg_r2)

# Run 10-fold cross-validation for both dependent variables using SVR
#cross_validate_svr(X_scaled, y1_scaled, dVar1, kernel='rbf', C=1.0, epsilon=0.1)
cross_validate_svr(X_scaled, y2_scaled, dVar2, kernel='rbf', C=1.0, epsilon=0.1)
