...
@author: Sami (Updated for 10-Fold Cross-Validation)
"""
...
def cross_validate_model(X, y, target_name, alpha=1.0, n_splits=10):
    ...
    print(f"\n10-Fold Cross-Validation Results for {target_name} (Ridge Regression, Alpha={alpha}):")
    print(f"Average MSE: {avg_mse:.4f}")
    print(f"Average R²: {avg_r2:.4f}")

    # Save results to CSV
    save_results_to_csv(f'Ridge Regression (10-Fold CV, Alpha={alpha})', target_name, avg_mse, avg_r2)
...
# cross_validate_model(X, y1, dVar1, alpha=1.0)
cross_validate_model(X, y2, dVar2, alpha=1.0)
