# -*- coding: utf-8 -*-
"""
Created on Fri Jan 31 07:56:56 2025
...
# Function to fit and evaluate a Gradient Boosting model
def fit_and_evaluate_gradient_boosting(X_train, X_test, y_train, y_test, target_name, n_estimators=100, learning_rate=0.1, max_depth=3):
    """
    Fits a Gradient Boosting regression model and evaluates its performance.
    ...
    Parameters:
        ...
        max_depth: Maximum depth of each individual tree (default: 3)
    """
    ...
    print(f"\nGradient Boosting Model for {target_name} (n_estimators={n_estimators}, learning_rate={learning_rate}, max_depth={max_depth})")
    print(f"Mean Squared Error (MSE): {mse:.4f}")
    print(f"R-squared (R2): {r2:.4f}")

    # Save results to CSV
    save_results_to_csv('Gradient Boosting Regression', target_name, mse, r2)
    ...
    feature_importance.to_csv(f"{target_name.replace(' ', '_').lower()}_gradient_boosting_feature_importance.csv", index=False)

# Fit and evaluate Gradient Boosting models
#fit_and_evaluate_gradient_boosting(X1_train, X1_test, y1_train, y1_test, dVar1, n_estimators=100, learning_rate=0.1, max_depth=5)
fit_and_evaluate_gradient_boosting(X2_train, X2_test, y2_train, y2_test, dVar2, n_estimators=100, learning_rate=0.1, max_depth=5)
