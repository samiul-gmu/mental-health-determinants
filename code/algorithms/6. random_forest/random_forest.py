# -*- coding: utf-8 -*-
"""
Created on Fri Jan 31 07:54:17 2025
...
# Function to fit and evaluate a Random Forest model
def fit_and_evaluate_random_forest(X_train, X_test, y_train, y_test, target_name, n_estimators=100, max_depth=None, min_samples_split=2):
    """
    Fits a Random Forest regression model and evaluates its performance.
    ...
    print(f"\nRandom Forest Model for {target_name} (n_estimators={n_estimators}, max_depth={max_depth}, min_samples_split={min_samples_split})")
    print(f"Mean Squared Error (MSE): {mse:.4f}")
    print(f"R-squared (R2): {r2:.4f}")

    # Save results to CSV
    save_results_to_csv('Random Forest Regression', target_name, mse, r2)
    ...
    feature_importance.to_csv(f"{target_name.replace(' ', '_').lower()}_random_forest_feature_importance.csv", index=False)

# Fit and evaluate Random Forest models
# fit_and_evaluate_random_forest(X1_train, X1_test, y1_train, y1_test, dVar1, n_estimators=100, max_depth=10, min_samples_split=5)
fit_and_evaluate_random_forest(X2_train, X2_test, y2_train, y2_test, dVar2, n_estimators=100, max_depth=10, min_samples_split=5)
