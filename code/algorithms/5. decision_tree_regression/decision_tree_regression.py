...
# Function to fit and evaluate a Decision Tree model
def fit_and_evaluate_decision_tree(X_train, X_test, y_train, y_test, target_name, max_depth=None, min_samples_split=2):
    """
    Fits a Decision Tree regression model and evaluates its performance.
    ...
    print(f"\nDecision Tree Model for {target_name} (max_depth={max_depth}, min_samples_split={min_samples_split})")
    print(f"Mean Squared Error (MSE): {mse:.4f}")
    print(f"R-squared (R2): {r2:.4f}")

    # Save results to CSV
    save_results_to_csv('Decision Tree Regression', target_name, mse, r2)
    ...
    feature_importance.to_csv(f"{target_name.replace(' ', '_').lower()}_decision_tree_feature_importance.csv", index=False)

# Fit and evaluate Decision Tree models
# fit_and_evaluate_decision_tree(X1_train, X1_test, y1_train, y1_test, dVar1, max_depth=5, min_samples_split=5)
fit_and_evaluate_decision_tree(X2_train, X2_test, y2_train, y2_test, dVar2, max_depth=5, min_samples_split=5)
