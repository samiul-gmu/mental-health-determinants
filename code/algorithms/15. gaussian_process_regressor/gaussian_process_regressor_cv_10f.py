...
@author: Sami (Updated for 10-Fold Cross-Validation with Gaussian Process Regression)
"""
...
# Function to perform 10-fold cross-validation using Gaussian Process Regression
def cross_validate_gpr(X, y, target_name, kernel=None, alpha=1e-10, n_splits=10):
    ...
    print(f"\n10-Fold Cross-Validation Results for Gaussian Process Regression ({target_name}, Kernel={kernel}):")
    print(f"Average MSE: {avg_mse:.4f}")
    print(f"Average R²: {avg_r2:.4f}")

    # Save results to CSV
    save_results_to_csv('Gaussian Process Regression (10-Fold CV)', target_name, avg_mse, avg_r2)

# Run 10-fold cross-validation for both dependent variables using Gaussian Process Regression
cross_validate_gpr(X_scaled, y1_scaled, dVar1)
cross_validate_gpr(X_scaled, y2_scaled, dVar2)
