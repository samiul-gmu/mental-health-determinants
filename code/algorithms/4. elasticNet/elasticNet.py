# -*- coding: utf-8 -*-
"""
Created on Fri Jan 31 07:46:42 2025
...
# Function to fit ElasticNet Regression, calculate B, SE, and evaluate model
def fit_and_evaluate_elasticnet(X_train, X_test, y_train, y_test, target_name, alpha=1.0, l1_ratio=0.5):
    """
    Fits an Elastic Net regression model, calculates coefficients (B) and standard errors (SE),
    ...
    Parameters:
        ...
        l1_ratio: Balance between L1 (Lasso) and L2 (Ridge) regularization (default: 0.5)
    """
    
    # Fit the Elastic Net model
    model = ElasticNet(alpha=alpha, l1_ratio=l1_ratio, max_iter=10000)  # max_iter ensures convergence
    ...
    print(f"\nElasticNet Regression Model for {target_name} (Alpha={alpha}, L1 Ratio={l1_ratio})")
    print(f"Mean Squared Error (MSE): {mse:.4f}")
    print(f"R-squared (R2): {r2:.4f}")

    # Save results to CSV
    save_results_to_csv('ElasticNet Regression', target_name, mse, r2)
    ...
    results['ElasticNet Effect'] = ['Eliminated' if b == 0 else 'Kept' for b in coefficients]

    print("\nModel Coefficients with Standard Errors (ElasticNet):")
    print(results)

    # Save results to CSV
    results.to_csv(f"{target_name.replace(' ', '_').lower()}_elasticnet_coefficients_se.csv", index=False)

# Fit and evaluate ElasticNet Regression models
# fit_and_evaluate_elasticnet(X1_train, X1_test, y1_train, y1_test, dVar1, alpha=1.0, l1_ratio=0.5)
fit_and_evaluate_elasticnet(X2_train, X2_test, y2_train, y2_test, dVar2, alpha=1.0, l1_ratio=0.5)
