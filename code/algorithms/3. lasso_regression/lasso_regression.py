# -*- coding: utf-8 -*-
"""
Created on Fri Jan 31 07:42:22 2025
...
# Function to fit Lasso Regression, calculate B and SE
def fit_and_evaluate_lasso(X_train, X_test, y_train, y_test, target_name, alpha=1.0):
    """
    Fits a Lasso regression model, calculates coefficients (B) and standard errors (SE),
    ...
    print(f"\nLasso Regression Model for {target_name} (Alpha={alpha})")
    print(f"Mean Squared Error (MSE): {mse:.4f}")
    print(f"R-squared (R2): {r2:.4f}")

    # Save results to CSV
    save_results_to_csv('Lasso Regression', target_name, mse, r2)
    ...
    results['Lasso Effect'] = ['Eliminated' if b == 0 else 'Kept' for b in coefficients]

    print("\nModel Coefficients with Standard Errors (Lasso):")
    print(results)

    # Save results to CSV
    results.to_csv(f"{target_name.replace(' ', '_').lower()}_lasso_coefficients_se.csv", index=False)

# Fit and evaluate Lasso Regression models
# fit_and_evaluate_lasso(X1_train, X1_test, y1_train, y1_test, dVar1, alpha=1.0)
fit_and_evaluate_lasso(X2_train, X2_test, y2_train, y2_test, dVar2, alpha=1.0)
