...
@author: Sami (Updated for 10-Fold Cross-Validation with Lasso Regression & Hyperparameter Tuning)
"""
...
# Define alpha range for Lasso tuning
...
# Function to perform Lasso Regression with hyperparameter tuning
def cross_validate_lasso_with_tuning(X, y, target_name, n_splits=10):
    kf = KFold(n_splits=n_splits, shuffle=True, random_state=42)

    # Perform Grid Search to find the best alpha
    lasso = Lasso(max_iter=10000)  # max_iter ensures convergence
    ...
    print(f"\n10-Fold Cross-Validation Results for Lasso Regression ({target_name}, Best α={best_alpha}):")
    print(f"Average MSE: {avg_mse:.4f}")
    print(f"Average R²: {avg_r2:.4f}")

    # Save results to CSV
    save_results_to_csv(f'Lasso Regression (10-Fold CV, Alpha={best_alpha})', target_name, avg_mse, avg_r2)
    ...
    results.to_csv(f"{target_name.replace(' ', '_').lower()}_lasso_coefficients_tuned.csv", index=False)

    return best_alpha, avg_mse, avg_r2

# Run Hyperparameter Tuning + 10-Fold CV for both dependent variables
# best_alpha_1, mse_1, r2_1 = cross_validate_lasso_with_tuning(X, y1, dVar1)
best_alpha_2, mse_2, r2_2 = cross_validate_lasso_with_tuning(X, y2, dVar2)
